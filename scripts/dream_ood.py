...
if platform.node() == 'lars-HP-ENVY-Laptop-15-ep0xxx':
    ...
    sys.path.append('/home/lars/Outliers/nonlinear-outlier-synthesis/file_size_estimation/')
else:
    sys.path.append('/storage/homefs/ld20d359/nonlinear-outlier-synthesis/')
    sys.path.append('/storage/homefs/ld20d359/nonlinear-outlier-synthesis/file_size_estimation/')
...
from file_size_estimation.fse.trainer import Trainer, _build_model, build_optimizer, build_engine, load, load_model_from_config
...
def get_class_names(opt):
    ...
    elif opt.id_data == 'c10':
        return ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse", "ship", "truck"]
    elif opt.id_data == 'oct':
        return ["OCT scan with retinal fluid", "OCT scan without retinal fluid"]
    elif opt.id_data == 'drd':
        return ["healthy retina", "mild diabetic retinopathy", "moderate diabetic retinopathy"]
    elif opt.id_data == 'astro':
        return ["galaxy", "quasar"]
    else:
        return [
    'apples',  # id 0
    'aquarium fish',
...
def get_prompt(opt, class_idx):
    chozen_class = get_class_names(opt)[class_idx % len(get_class_names(opt))]
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="complexity guidance scale",
    )
    parser.add_argument(
        "--clip_embs",
        action='store_true',
        help="Use CLIP embeddings instead of modifying first class token",
    )
    ...
    if opt.guid_ckpt is not None:
        with open("file_size_estimation/params.yml", 'r') as f:
            params = yaml.safe_load(f)

        params["use_logger"] = False
        
        input_dimensionality = (4, 64, 64)
        print(f"{input_dimensionality} dimensions")

        fse_model = _build_model(input_dimensionality, params)

        optimizer_staff = build_optimizer(params, fse_model, [])
        optimizer = optimizer_staff['optimizer']
        lr_scheduler = optimizer_staff['lr_scheduler']

        trainer = Trainer(fse_model, None, 1.0, optimizer, lr_scheduler)
        engine = build_engine(trainer, "", None, None, params=params)

        load(opt.guid_ckpt, trainer=trainer, engine=engine)

        guidance_model = trainer.model
    else:
        guidance_model = None

    desired_complexity = torch.tensor([2.0], device=device) / 4 - 1

    if opt.clip_embs:
        embs = torch.from_numpy(np.load(opt.loaded_embedding))
            ...
            with model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        prompts, chozen_class = get_prompt(opt, class_idx)
                        print(prompts)
                        ...
                        if opt.clip_embs:
                            c = embs[class_idx][np.random.choice(embs.shape[1], opt.n_samples, replace=False)].cuda()
                        else:
                        ...
                        samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                         ...
                                                         eta=opt.ddim_eta,
                                                         x_T=start_code,
                                                         comp_guidance_strength=opt.guid_str,
                                                         comp_guidance_model=guidance_model,
