...
parser.add_argument('--ood_size', type=int, default=6400)
parser.add_argument('--ood_transform', type=str, default='both')
# WRN Architecture
...
if args.ood_transform == 'both':
    ood_transform = trn.Compose([trn.ToTensor(), trn.ToPILImage(),
                                    v2.RandomChoice([
                                        trn.Resize(32, interpolation=InterpolationMode.NEAREST),
                                        trn.Resize(32, interpolation=InterpolationMode.BILINEAR)
                                    ]),
                                    trn.RandomCrop(32, padding=4),
                                    trn.RandomHorizontalFlip(), trn.ToTensor(),
                                    trn.Normalize(mean, std)])
elif args.ood_transform == 'bilinear':
    ood_transform = trn.Compose([trn.ToTensor(), trn.ToPILImage(),
                                    trn.Resize(32),
                                    trn.RandomCrop(32, padding=4),
                                    trn.RandomHorizontalFlip(), trn.ToTensor(),
                                    trn.Normalize(mean, std)])
elif args.ood_transform == 'nearest':
    ood_transform = trn.Compose([trn.ToTensor(), trn.ToPILImage(),
                                    trn.Resize(32, interpolation=InterpolationMode.NEAREST),
                                    trn.RandomCrop(32, padding=4),
                                    trn.RandomHorizontalFlip(), trn.ToTensor(),
...
ood_data = dset.ImageFolder(root=args.my_info, transform=ood_transform)
ood_data = torch.utils.data.Subset(ood_data, np.random.choice(len(ood_data), args.ood_size, replace=False))
...
def train(epoch):
    ...
    print(f"num_in_images: {num_in_images}, num_out_images: {num_out_images}")
