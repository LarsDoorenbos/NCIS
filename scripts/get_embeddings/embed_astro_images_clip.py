...
anchors = torch.from_numpy(np.load('./token_embed_astro.npy')).cuda()
class_names = ["galaxy", "quasar"]
...
train_data_in = torchvision.datasets.ImageFolder(root='/storage/workspaces/artorg_aimi/ws_00000/lars/astronomy/in/', transform=transform)
train_data_in, test_data = torch.utils.data.random_split(train_data_in, [len(train_data_in) - 200, 200], generator=torch.Generator().manual_seed(1))

print(len(train_data_in), len(test_data))
...
np.save(f"embed_astro_images_clip_{args.offset}_{args.num_images}.npy", image_embeds)
np.save(f"labels_astro_images_clip_{args.offset}_{args.num_images}.npy", labels)
