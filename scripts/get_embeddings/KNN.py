...
def KNN_dis_search_decrease(target, index, K=50, select=1,shift=0, normalize=True):
    '''
...
def KNN_dis_search_distance(target, index, K=50, num_points=10, length=2000,depth=342,shift=0, normalize=True, fix=False):
    '''
    data_point: Queue for searching k-th points
    target: the target of the search
    K
    ...
    if not fix:
        ...
        k_th = k_th_distance.view(-1, length)
        if shift:
            k_th_distance, minD_idx = torch.topk(-k_th, num_points, dim=1)
        else:
            k_th_distance, minD_idx = torch.topk(k_th, num_points, dim=1)
        ...
        if len(minD_idx.size()) == 1:
            minD_idx = minD_idx.reshape(-1,1)
        for i in range(minD_idx.shape[0]):
            point_list.append(i*length + minD_idx[i])
...
def generate_outliers(ID, input_index, negative_samples, ID_points_num=2,
                      ...
                      pic_nums=30, depth=342, shift=0, normalize=True, fix=False):
    ...
    minD_idx, k_th = KNN_dis_search_decrease(ID, index, K, select, shift=shift, normalize=normalize)
    ...
    # breakpoint()
    point = KNN_dis_search_distance(negative_sample_list, index, K, ID_points_num, length,depth, shift=shift, normalize=normalize, fix=fix)
