...
anchors = torch.from_numpy(np.load('./token_embed_oct.npy')).cuda()
class_names = ["OCT scan with retinal fluid", "OCT scan without retinal fluid"]
...
traindir = "/storage/research/artorg_aimi/ld20d359/home/ubelix/artorg/shared/for_lars/corruption/training_datasets/oct_bscanwise/training"
...
# Shuffle the dataset
train_data_in = torch.utils.data.DataLoader(train_data_in, batch_size=1, shuffle=True)
...
np.save \
        (f"embed_oct_images_clip_{args.offset}_{args.num_images}.npy", image_embeds)
