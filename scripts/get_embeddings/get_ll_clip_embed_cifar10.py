...
if sum_temp == num_classes * 5000:
    for index in range(num_classes):
        ...
        negative_samples = new_dis.rsample((20000,))

        prob_density = new_dis.log_prob(negative_samples)

        _, index_prob = torch.topk(-prob_density, 1000)
...
np.save \
        ('./cifar10_outlier_ll_clip_noise_' + str(args.gaussian_mag_ood_det) + '.npy', torch.stack(ood_samples).cpu().data.numpy())
