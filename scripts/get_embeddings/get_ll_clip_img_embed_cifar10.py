...
if sum_temp == num_classes * 5000:
    for index in range(num_classes):
        ...
        negative_samples = new_dis.rsample((len(class_embs) * 20,))

        prob_density = new_dis.log_prob(negative_samples)

        _, index_prob = torch.topk(-prob_density, len(class_embs))
...
np.save \
        ('./cifar10_outlier_ll_img_clip_noise_' + str(args.gaussian_mag_ood_det) + '.npy', torch.stack(ood_samples).cpu().data.numpy())
