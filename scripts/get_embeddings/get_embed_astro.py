...
parser.add_argument('--K_in_knn', type=int, default=200)
...
anchor = torch.from_numpy(np.load('./token_embed_astro.npy')).cuda()
num_classes = 2
sum_temp = 0
data_dict = torch.from_numpy(np.load('./id_feat_astro_199epoch.npy')).cuda()
...
if args.shift:
    np.save \
        ('./astro_inlier_npos_embed'+ '_noise_' + str(args.gaussian_mag_ood_gene)  + '_select_'+ str(
        args.ood_gene_select) + '_KNN_'+ str(args.K_in_knn) + '.npy', torch.stack(ood_samples).cpu().data.numpy())
else:
    np.save \
        ('./astro_outlier_npos_embed' + '_noise_' + str(args.gaussian_mag_ood_det) + '_select_' + str(
